...
interface Gauge { series: Point[]; latest: Point; unit: string }

async function fetchGauge(site: string, param: string, period?: string): Promise<Gauge> {
...
function flowContext(val: number): { ctx: string; col: string; pct: number } {
...
function spawnPhase(): { label: string; unit: string; ctx: string; col: string } {
  const now = new Date()
